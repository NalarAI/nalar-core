...
BARIS = [
    ...
    ("rp@1000 K2+K3", "metrik_dengan_k3.nalar_k2_plus_k3.rupiah_pada_k.1000", "rp"),
    ("rp@1000 K3 saja", "metrik_dengan_k3.nalar_k3_saja.rupiah_pada_k.1000", "rp"),
    ("rp@1000 mesin aturan", "metrik.mesin_aturan.rupiah_pada_k.1000", "rp"),
    ("rp@1000 regresi berlabel", "metrik.regresi_logistik.rupiah_pada_k.1000", "rp"),
    ("presisi@1000 nalar", "metrik.nalar.presisi_pada_k.1000", None),
    (
        "lift@50 atas aturan",
        "metrik_dengan_k3.nalar_k2_plus_k3.peningkatan_atas_aturan.50",
        None,
    ),
    (
        "lift@1000 atas aturan",
        "metrik_dengan_k3.nalar_k2_plus_k3.peningkatan_atas_aturan.1000",
        None,
    ),
    ...
    (
        ...
        ["konformal", "0.01", "laju_penandaan_klaim_bersih"],
        None,
    ),
    ("konformal a=0.01 lulus", ["konformal", "0.01", "lulus"], None),
    (
        "rp@1000 nilai klaim saja",
        ["metrik", "nilai_klaim", "rupiah_pada_k", "1000"],
        "rp",
    ),
    (
        ...
        [
            "metrik_dengan_k3",
            "nalar_k2_plus_k3",
            "peningkatan_atas_nilai_klaim",
            "1000",
        ],
        None,
    ),
    ("KEADILAN rasio maks min", "keadilan.menurut_kelas_faskes._rasio_maks_min", None),
    (
        "KEADILAN lulus batas 2x",
        "keadilan.menurut_kelas_faskes._lulus_batas_dua_kali",
        None,
    ),
    ("pelaku hati hati ambil", "adversarial.hati_hati.total_diambil_rp", "rp"),
    ("pelaku hati hati maks/klaim", "adversarial.hati_hati.maks_per_klaim_rp", "rp"),
    ("T6 penurunan maks", "adversarial._penurunan_keuntungan_maksimum", None),
...
def utama(jalur_berkas: list[str]) -> int:
    ...
    for label, jalur, fmt in BARIS:
        ...
        for d in data:
            v = ambil(d, jalur)
            sel.append(rp(v) if fmt == "rp" else ("-" if v is None else str(v)))
