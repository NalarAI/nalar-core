...
from nalar.profil import (peringkat_faskes, presisi_faskes_pada_k,  # noqa: E402
                          profil_faskes)
...
def utama(n_peserta=20000, tahun=3, seed=7, keluaran="runs/profil.json"):
    ...
    for r, s in zip(e_te, sel):
        ...
        nilai_total[k] = nilai_total.get(k, 0.0) + r["tarif"] + \
            r.get("tagih_bhp", 0)
    ...
    print(f"    {len(prof)} faskes bisa dinilai, "
    ...
    for i, r in enumerate(e_te):
        per_faskes.setdefault((int(r["f_jenis"]), int(r["faskes"])),
                              []).append(i)
    urut_mentah = sorted(
        dinilai, key=lambda k: -float(np.mean(sel[per_faskes[k]])))
    ...
    for nama in ("susut", "z_susut", "mentah", "volume"):
        baris = "  ".join(
            ...
            f" ({hasil[nama].get(f'peningkatan@{k}')}x)" for k in daftar_k)
    ...
    for nama, urut in (("susut", urut_susut), ("mentah", urut_mentah)):
        ...
        print(f"    {nama:8s} klaim rata rata di 25 teratas {v:7.1f} "
    ...
    for j, nm in enumerate(nama_keb):
        ...
        tangkap[nm] = {"ada": len(punya), "di_25_teratas": ada,
                       "tertangkap": round(ada / len(punya), 3)}
        print(f"    {nm:11s} ada {len(punya):3d}, "
    ...
    catatan["contoh_baris_teratas"] = [
        {"faskes": f"{k[0]}:{k[1]}", **v} for k, v in
        peringkat_faskes(prof, atas=5)]
    catatan["putusan"] = {
        "susut_mengalahkan_mentah": bool(
            hasil["susut"].get("presisi@25", 0)
            > hasil["mentah"].get("presisi@25", 0)),
        "susut_mengalahkan_volume": bool(
            hasil["susut"].get("presisi@25", 0)
            > hasil["volume"].get("presisi@25", 0)),
