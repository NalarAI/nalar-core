...
with torch.no_grad():
    att.bias_bidang.copy_(torch.arange(2 * N_FIELDS * N_FIELDS,
                                       dtype=torch.float32)
                          .view(2, N_FIELDS, N_FIELDS))
...
cek("nilai bias diambil dari pasangan bidang yang benar",
...
cek("saklar bias bisa dimatikan untuk ablasi",
...
for alpha in (0.01, 0.05, 0.10):
    ...
    cek(f"alpha {alpha}: laju penandaan {rata_laju:.4f} tidak melebihi alpha",
        rata_laju <= alpha * 1.25, f"terukur {rata_laju:.4f}")

cek("kalibrator per kelompok memakai ambang berbeda tiap kelompok",
    len(Kalibrator(0.05, minimal_kelompok=50).pasang(
        rng.normal(size=1000), rng.integers(0, 3, 1000)).ambang_kelompok) == 3)

# ---------------------------------------------------------------------------
print("\n3. Aturan tarif berperilaku seperti INA-CBG")
# ---------------------------------------------------------------------------
cek("tanpa diagnosis sekunder berat, keparahan I",
    hitung_keparahan([], 4, True) == 1)
cek("satu diagnosis sekunder berat menaikkan ke keparahan II",
    hitung_keparahan(["N17"], 4, True) == 2)
cek("dua diagnosis sekunder berat menaikkan ke keparahan III",
    hitung_keparahan(["N17", "J96"], 4, True) == 3)
cek("rawat jalan tidak punya tingkat keparahan",
...
cek("menambah dua diagnosis berat menaikkan tarif",
    t3 > t1, f"{t1} -> {t3}")
cek("selisih keparahan I ke III berada di kisaran jutaan rupiah",
    2_000_000 < (t3 - t1) < 12_000_000, f"selisih {t3 - t1}")
...
cek("leukosit 22 ribu masuk pita sangat tinggi",
    K.pita_lab("LEUKO", 22000.0) >= 5)
cek("dua pemeriksaan berbeda dengan posisi relatif sama masuk pita sama",
...
cek("porsi klaim terpengaruh berada di rentang literatur tiga sampai lima belas persen",
    0.03 <= porsi <= 0.15, f"terukur {porsi:.3f}")
...
cek("tidak ada faskes yang muncul di latih dan uji sekaligus",
